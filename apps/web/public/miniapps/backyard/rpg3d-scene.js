 ...
 * @param {{walking?:boolean,onError:(error:unknown)=>void}} options */
create(host,canvas,options) {
  ...
  try {
    ...
    /** 고정 방위각으로 추종하고 수관 가림만 줄인다. @param {{x:number,y:number,z:number}} point @param {number} direction @param {number} delta @param {boolean} [snap] */
    function update(point,direction,delta,snap=false){
    ...
    return {dispose,resize,update,render};
