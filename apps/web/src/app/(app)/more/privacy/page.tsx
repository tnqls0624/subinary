 ...
 *
 * ⚠️ 이 화면은 **삭제를 약속하지 않는다.** 원문 purge 잡·보존기간 선택은 **만들지
 * 않기로 했다**(2026-08-18 사용자 결정, 로드맵 5-1). 보존정책은 `none` 동결이다.
 * "N일 뒤 삭제"도 "곧 됩니다"도 쓰지 않는다 — 지키지 못할 약속과 **하지 않을 약속**은
 * 똑같이 거짓말이고, 후자가 더 오래 간다. 보관은 **지금 상태를 그대로 보여주기만** 한다.
 * 다시 볼 조건은 외부 사용자 개방 · 법정 요구 · 저장량 임계뿐이다.
 *
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
      ...
      <Card>
        <CardContent className="flex flex-col gap-4">
          ...
          {/*
           * 지키지 못할 약속을 하지 않기 위한 문단이다. 예전엔 "삭제는 준비 중"이라고
           * 썼는데, 그건 **하지 않기로 결정한 일**을 곧 한다고 말하는 것이었다. 없다는
           * 사실과 그 이유를 함께 말한다 — 이유 없는 부재는 게으름으로 읽히지만,
           * 이유 있는 부재는 판단으로 읽힌다.
           */}
          <p className="text-muted-foreground bg-muted rounded-xl p-4 text-[13px] leading-relaxed">
            동의를 철회하면 새 문자 수집은 즉시 멈춰요. 다만{" "}
            <strong className="font-medium">
              이미 보관된 문자 원문은 그대로 남아요.
            </strong>{" "}
            자동 삭제와 보관 기간 설정은 만들지 않기로 했어요 — 문자 원문은 금액이
            틀렸을 때 다시 읽어 바로잡는 유일한 근거라서, 지우면 과거 지출이 맞는지
            확인할 방법이 함께 사라지거든요. 지울 필요가 생기면 알려 주세요.
          </p>
        </CardContent>
      </Card>
