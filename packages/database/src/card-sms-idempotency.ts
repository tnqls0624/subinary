 ...
 *   연결 복구 직후 재발화다. 같은 SMS가 OS/RCS 중복 알림으로 두 번 트리거되는 경우도
 *   초 단위다.
 *   ⚠️ **이 상한은 실측으로 깨졌다**(2026-08). 08-23 20:05 쿠팡 취소 문자 하나가
 *   08-24 07:44(11시간 뒤)와 08-25 09:38(25시간 뒤)에 재전송돼 `content_hash`가
 *   같은 이벤트 3건이 됐다. 자동화가 밀린 문자를 나중에 몰아 보내면 간격은 분이
 *   아니라 **시간·일 단위**가 된다. 아래 "지문 창을 따로 두는 이유" 참조.
 ...
 * 한다.
 *
 * ## 지문 창을 키 창과 분리한 이유 (2026-08)
 *
 * 위 실측이 보여준 것은 "3분이 좁다"가 아니라 **두 창이 같은 값일 이유가 없다**는
 * 것이다. 중복 판정은 두 단계로 일어난다.
 *
 * 1. `UNIQUE(device_id, event_id)` — 키가 같아야 걸린다. 키에 창을 섞으므로 이
 *    단계의 유효 범위는 창 크기다.
 * 2. 지문(`content_hash`) 조회 — 키가 달라도 **같은 원문**이면 걸린다.
 *    ({@link CardSmsIdempotency.windowLowerBound})
 *
 * 1의 창을 키우면 `event_id`의 의미가 바뀌고(위 문단) 기존 행과의 호환이 깨진다.
 * 반면 2는 조회 범위일 뿐이라 넓혀도 키 형식이 그대로다. 그래서 **키 창은 3분으로
 * 두고 지문 창만 넓힌다.**
 *
 * 다만 무조건 넓히면 "같은 문자를 만든 별개 결제"를 삼킨다. 그 위험은 본문이 스스로
 * 시간을 담고 있는지에 달려 있다 — 국내 카드사 문자는 대개 `MM/DD HH:mm`을 포함하므로
 * **본문이 같다는 것 자체가 같은 거래라는 뜻**이고, 다른 거래는 시각이 달라 지문이
 * 갈린다. 그래서 본문에 일시가 있을 때만 {@link CARD_SMS_FINGERPRINT_WINDOW_MS}로
 * 넓히고, 타임스탬프 없는 짧은 문자는 종전대로 3분을 쓴다(넓히면 진짜 소실이 된다).
 */
...
/**
 * 본문이 거래 일시를 담고 있을 때 쓰는 **지문 조회 창**(밀리초). 키 창과 달리
 * `event_id`의 의미를 바꾸지 않으므로(파일 상단 "지문 창을 키 창과 분리한 이유")
 * 넓혀도 기존 행과 호환된다.
 *
 * 30일인 이유: 실측된 재전송 지연은 최대 25시간이었고, 자동화가 한 달치를 몰아 보내는
 * 경우까지 덮으면 충분하다. 무한이 아니라 상한을 두는 것은 `(device_id, content_hash,
 * received_at)` 인덱스 스캔 범위를 묶어 두기 위함이고, 정기결제처럼 **한 달 주기로
 * 같은 가맹점·같은 금액**이 반복되는 문자는 본문의 일시가 달라 애초에 지문이 갈린다.
 */
export const CARD_SMS_FINGERPRINT_WINDOW_MS = 30 * 24 * 60 * 60 * 1000;

/**
 * 본문에 `MM/DD HH:mm` 류의 거래 일시가 있는지 — 지문 창을 넓혀도 되는지의 판정.
 *
 * 있으면 "본문이 같다 = 같은 거래"가 성립한다(다른 거래는 시각이 달라 본문이 갈린다).
 * 없으면 성립하지 않으므로 창을 넓히면 안 된다 — 타임스탬프 없는 짧은 문자는 서로 다른
 * 결제가 바이트 단위로 같을 수 있고, 그때 넓은 창은 중복 흡수가 아니라 **결제 소실**이다.
 *
 * 연/월/일 접두(`2026/08/20 23:54`)와 구분자 `/ . -`를 모두 받는다. 시각(`HH:mm`)을
 * 반드시 요구하는 이유: 날짜만으로는 같은 날 같은 금액의 별개 결제를 가르지 못한다.
 */
export function cardSmsContentCarriesTimestamp(content: string): boolean {
  return /(?:\d{2,4}[./-])?\d{1,2}[./-]\d{1,2}\s+\d{1,2}:\d{2}/.test(content);
}
...
export function resolveCardSmsIdempotency(
  ...
  return {
    ...
    contentHash,
    // 키 창(3분)과 지문 창은 다르다. 본문이 스스로 시간을 담고 있으면 몇 시간·며칠 뒤
    // 재전송도 같은 거래이므로 넓게 본다 — 실측된 11시간·25시간 지연 재전송이 여기서
    // 걸린다. 시간 축이 없는 본문은 넓히면 별개 결제를 삼키므로 종전 3분을 쓴다.
    windowLowerBound: cardSmsContentCarriesTimestamp(input.content)
      ? new Date(input.ingestedAt.getTime() - CARD_SMS_FINGERPRINT_WINDOW_MS)
      : cardSmsDedupeWindowLowerBound(input.ingestedAt),
  };
}
