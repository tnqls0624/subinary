...
> [ADR-0016 토큰 수집 경로](../adr/0016-device-token-ingest.md) ·
> [Phase 3 빌드 스펙](../phase3-build-spec.md) · [장치/HMAC API](./devices.md)

## 개요

Phase 3은 다음 진입점을 제공한다. **수집은 두 가지 인증 경로**(HMAC 서명 · 토큰 Bearer)가
병행하며, 둘 다 동일한 `CardSmsIngestService.ingest`로 처리된다.

| 대상 | 방식 | 경로 | 쓰임 |
|---|---|---|---|
| 장치(스마트폰 앱) | HMAC-SHA256 서명 | `POST /v1/mobile-events/card-sms` | 카드 문자 **수집**(원문 업로드) |
| 장치(단축어/MacroDroid) | 수집 토큰 `Authorization: Bearer` | `POST /v1/mobile-events/card-sms-token` | 카드 문자 **수집**(저마찰 자동화) |
| 사용자(사람) | JWT Access Token | `GET /v1/card-sms-events[...]` | 수집·파싱 결과 **조회** |

- HMAC 수집은 Phase 2의 `DeviceHmacGuard`(ADR-0007)를 그대로 재사용한다 — 서명 프로토콜은
  [devices.md §2](./devices.md)와 동일하다.
- 토큰 수집은 단축어(iOS)/MacroDroid(Android)처럼 **HMAC 서명을 계산하기 어려운 자동화 도구**를
  위한 저마찰 경로다(§1-토큰, [ADR-0016](../adr/0016-device-token-ingest.md)). "고정 헤더 +
  JSON POST"만으로 동작하며, 처리 파이프라인은 HMAC 경로와 완전히 동일하다.
...
## 1-토큰. 수집(토큰) — `POST /v1/mobile-events/card-sms-token` (단축어/MacroDroid)

단축어(iOS)/MacroDroid(Android)는 요청 본문에 대한 HMAC-SHA256 서명을 계산하기 어렵다. 이
경로는 **장치별 수집 토큰(Bearer)** 하나로 인증해, "고정 헤더 + JSON POST"만으로 카드 문자를
수집한다(설계 근거: [ADR-0016](../adr/0016-device-token-ingest.md)). 전역 `AccessTokenGuard`는
`@Public()`로 우회하고 `DeviceTokenGuard`가 토큰을 검증한다. **수집 이후 처리(원문 이중보존·
비동기 파싱·조회·멱등)는 §1 HMAC 경로와 완전히 동일**하며, 같은 `CardSmsIngestService.ingest`를
공유한다.

### 수집 토큰 발급 (등록/회전 응답의 `collectToken`)

- 장치 등록(`POST /v1/devices/register`)·회전(`POST /v1/devices/:id/rotate-secret`) 응답에
  `collectToken`(raw)이 **1회만** 포함된다(HMAC `secret`과 함께 나옴 — 두 경로 병행).
- 서버는 토큰 원문을 저장하지 않고 `sha256(collectToken)` 해시만 보관한다(`registered_devices.
  collect_token_hash`). 응답을 놓쳤다면 **다시 조회할 수 없으므로** rotate-secret으로 재발급한다.
- rotate-secret은 HMAC `secret`과 `collectToken`을 **둘 다** 새로 발급한다(옛 토큰 즉시 무효).
- 장치 폐기(`DELETE /v1/devices/:id`)하면 `status='revoked'`가 되어 토큰도 즉시 무효가 된다.

### 요청

| 헤더 | 값 |
|---|---|
| `Authorization` | `Bearer <collectToken>` (등록/회전 시 받은 raw 토큰) |
| `Content-Type` | `application/json` |

body는 §1과 **동일한** `cardSmsIngestRequestSchema`다(서명이 없으므로 rawBody 바이트 일치 제약
없음 — 재직렬화해도 무방):
...
```bash
COLLECT_TOKEN="a1b2…"                  # 등록/회전 시 1회 노출된 raw collectToken
curl -s -i -X POST http://localhost:3001/v1/mobile-events/card-sms-token \
  -H "Authorization: Bearer $COLLECT_TOKEN" \
  -H 'Content-Type: application/json' \
  --data '{"eventId":"b3f1c2a4-…","sender":"15447200","content":"신한카드 승인 12,500원 일시불 07/15 19:32 스타벅스","receivedAt":"2026-07-15T10:32:00.000Z"}'
```
...
오류 규칙(토큰 인증 실패는 원인 비노출 일반 `401`):

| 상황 | 응답 |
|---|---|
| `Authorization: Bearer` 헤더 없음 / 형식 오류 | `401` |
| 토큰 해시가 어느 활성 장치와도 매칭 안 됨(잘못된/옛/미발급 토큰) | `401` |
| 장치가 폐기됨(`status='revoked'`) | `401` |
| body 스키마 위반(필드 누락·길이 초과 등) | `400`(zod) |
| 본문 크기 초과(`MOBILE_MAX_BODY_BYTES`) | `413`(Fastify) |
...
| 항목 | HMAC 경로 (§1) | 토큰 경로 (§1-토큰) |
|---|---|---|
| 인증 | 요청별 `X-Signature`(HMAC-SHA256) | 고정 `Authorization: Bearer` |
| 자격증명 저장 | AES-256-GCM 암호화(재계산 필요) | `sha256` 해시(원문 미저장) |
| 본문 무결성 | 서명으로 검증(rawBody 바이트 일치) | 없음(토큰 비밀성에 의존) |
| per-request replay 방어 | timestamp + nonce UNIQUE | 없음 — `eventId` 멱등으로 재전송 무해화 |
| 대상 | HMAC 계산 가능한 앱 | 단축어/MacroDroid 등 저마찰 도구 |

- 토큰 경로는 서명·nonce·timestamp가 없어 **per-request replay를 nonce로 막지 않는다**. 대신
  `UNIQUE(deviceId, eventId)` 멱등으로 동일 문자 재전송/재생을 무해화한다(중복 저장 없음).
- 남는 위협은 **위조 문자 주입**이며, 이는 전적으로 **토큰 비밀성(HTTPS)**에 의존한다. 유출이
  의심되면 `rotate-secret`(새 토큰)·`revoke`(폐기)로 즉시 대응한다.
- 서명이 가능한 장치는 보안이 더 강한 §1 HMAC 경로를 계속 쓰는 것을 권장한다.
...
토큰 수집 경로(§1-토큰)는 `scripts/verify-card-sms-token.mjs`가 별도로 검증한다(addendum §6
시나리오 1~7). 서명 없이 `Authorization: Bearer <collectToken>`으로 신한 승인 문자를 수집해
`200 queued` → 폴링 후 `parsed` → 동일 eventId 재전송 `duplicate:true`(중복 저장 없음) →
잘못된 토큰/Bearer 없음 `401` → rotate-secret 후 옛 토큰 `401`·새 토큰 `200` → revoke 후
`401`을 확인한다. 로그에 collect 토큰 원문/해시·secret·문자 원문을 출력하지 않는다.

```bash
node scripts/verify-card-sms-token.mjs
