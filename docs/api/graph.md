...
- **추출은 결정적 규칙 함수**(PRD §3.4, `@family/rag` `graph.ts`): 랜덤/시간/LLM 을 쓰지 않는다.
  ...
  - **Phase 9 축소(스펙 §5)**: `works_on`(person↔technology)은 chunk 에 author 정보가 없어 자동 생성하지
    ...
    하도록 순수 함수 경계를 유지한다.
- **자동 증분 추출**: RAG publish의 `rag.chunk.graph-ready.v1` event가 chunk revision target 잡을
  만들고 워커가 current revision을 재검증한다. `POST /extract`는 복구·extractor backfill용 전체 rebuild다.
...
- **멱등/계보**: entity canonical projection은 기존 unique key를 유지하고 원문 관측은
  `graph_entity_mentions`에 chunk revision/extractor version별로 기록한다. 자동 관계도 같은 provenance를
  가진 partial unique index로 중복을 막는다. 편집 전 mention/관계는 삭제하지 않고 `validUntil`로 마감하며,
  명시적 사용자 supersede 관계는 자동 reconcile에서 제외한다.
...
소유한 workspace 의 current chunk revision + `slack_users`를 다시 추출하는 복구/backfill 잡을 큐에 넣는다.
일상적인 메시지 변경은 자동으로 대상 revision만 처리한다.
  ...
  "neighbors": [
    {
      "relationship": {
        ...
        "extractorVersion": "graph-rule-v1",
