...
1. `docs/impl-backyard-context.md` — **이미 측정된 사실.** 환경·크기 기준값·확정 밸런스·
   완성된 파일·저장소 함정이 전부 여기 있다. **다시 재지 마라.**
2. `docs/impl-backyard-v0.md` **§7의 6·7행과 §8** — 이번 슬라이스의 완료 게이트와 위험.

그 외 문서는 **필요할 때만** 편다. context 문서의 「문서 지도」를 보라.
`research-game-library-2026-09.md`는 읽지 마라(엔진은 canvas로 결정됨).

## 이어받는 상태 — 슬라이스 6이 일부 진행됐다

앞 워커가 터미널 종료로 중단됐으나 **작업물은 남아 있고 깨지지 않았다.**
코디네이터가 확인함: vitest 138개 통과, `tsconfig.miniapps-dom.json` 통과, `node --check` 통과.

변경된 파일: `app.js`(7.1K→9.6K) · `renderer.js`(4.6K→6.0K) · `index.html` · `style.css`

**처음부터 다시 만들지 마라.** 현재 파일을 읽고 §7의 6행 게이트 중 무엇이 이미
충족됐는지 판단한 다음, 남은 것만 채워라.
...
1. **먼저** `docs/impl-backyard-s6-report.md`에 진행 상황을 쓴다. 게이트를 하나 통과할 때마다
   그 파일을 갱신한다. **중간에 끊겨도 결과가 남아야 한다.**
2. **마지막에** `worker_done`을 보낸다. 본문에 통과한 게이트와
   **새 OTA ZIP 크기 · 기준(1,123,874B) 대비 증가량**을 담는다.

앞 슬라이스에서 워커가 검증을 다 끝내고 `worker_done`을 보낼 크레딧이 없어 결과가
미아가 될 뻔했다. 보고서를 먼저 쓰는 것이 그 재발을 막는다.
