...
|---|---|---|---|
| C-3 Control Center 본체 (보존정책 선택 · 원문 purge 잡 · 삭제 이력) | L | 법적 보관·삭제 요구가 관할·운영 정책 검토를 필요로 한다. 8주에 불완전하게 밀어 넣으면 "지웠다고 했는데 남아 있다"가 되어 P0-2보다 나쁜 신뢰 사고가 된다 | **충족됨** — ADR-0028(마이그레이션 `0051`) 배포. 기획: `docs/concept-data-control-2026-08.md`(2026-08-18). 착수는 ADR-0027 금액 전환 완료 후 |
| owner 데이터 내보내기 · 가족 그룹 삭제 (PO B9) | L | 비동기 export·만료 링크·재인증·유예 기간·감사 이벤트가 한 묶음이다. 위 항목과 같은 데이터 모델을 쓰므로 따로 만들면 두 번 만든다 | 위와 같은 기획서에 S3·S4로 병합함(`docs/concept-data-control-2026-08.md`) |
