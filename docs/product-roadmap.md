...
| **2026-09-06** | **플레이그라운드를 게임으로 재출발.** 미니앱 런타임 구축 → 전부 삭제 → 라이브러리 조사·sandbox 실행 검증·게임 설계 완료 |
...
### 2026-09-06 — 플레이그라운드: 지은 것을 지우고 다시 시작했다

로드맵의 P0/P1 축 밖에서 진행한 별도 갈래다. 경위를 남기지 않으면 삭제 커밋
(`b88588a`)만 보고 "왜 지웠는지" 알 수 없다.

| 단계 | 무엇 | 결과 |
|---|---|---|
| 1 | 미니앱 런타임(격리 iframe + 브릿지 + 권한) 구축 | 배포됨 (`4d4b769`) |
| 2 | 정보성 화면 4종 + 게임 3종 | **게임이 아니었다** — 사용자 지적으로 구분이 드러남 |
| 3 | 전부 삭제 | `b88588a`. `play_states` 테이블·`/v1/play/*` API·훅은 **남김** |
| 4 | 라이브러리 조사 (Codex) | [research-game-library](./research-game-library-2026-09.md) — Phaser 4.2.1 추천 |
| 5 | sandbox 실행 검증 (Codex) | [verify-sandbox-engine](./verify-sandbox-engine-2026-09.md) — **조건부 가능** |
| 6 | 게임 설계 | [design-tycoon-game](./design-tycoon-game-2026-09.md) — "뒷마당 타이쿤" v0 |

**검증이 설계를 바꾼 지점**: `sandbox="allow-scripts"`는 `allow-same-origin`이 없어
문서가 불투명 origin이 된다. 같은 앱에 든 PNG·JSON도 상대경로로 못 읽는다(CORS,
XHR status 0). data URI와 캔버스 생성 텍스처는 통과한다. 그래서 v0의 "아트 이미지
0개"는 취향이 아니라 **현재 실행 조건이 강제하는 것**이다.

Excalibur는 차선 후보였으나 **로딩 실패 시 로더 Promise가 완료되지 않아** 오류 처리가
불가능하다. 대체재로서 값이 떨어진다.

| 남은 것 | 상태 |
|---|---|
| 미니앱 호스트 런타임 복원 | 3단계에서 클라이언트 런타임을 함께 지웠다. v0 선행 조건, 0.5~1일 |
| Capacitor 실기기 검증 | 로컬 scheme·CSP·실기기 터치 전부 **확인 못 함** |
| 자산 제공 경로 결정 | CORS 헤더를 붙이는 대조 시험 미실시. v0는 인라인으로 가능 |
...
| 정기 결제 **금액 레이어** (월 구독 총액 · 예정 알림 금액 · 해지 종료 처리 · 카드 교체 CTA) | M | 금액 위의 **예고**라서 ADR-0027 7단계(레거시 186건 수리)가 선행이었다 | **선행 조건 해소됨 (2026-09-03)** — 7단계 완료로 259건 전량이 v2 계약이다(delta 0, 월 합계 무변경). `net_amount`가 확정이라 금액 레이어를 열 수 있다. 남은 판단은 정밀도가 예고를 감당하는가뿐이고, 그건 확정 후보가 쌓여야 안다 |
