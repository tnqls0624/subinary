...
- **핵심 시나리오**: 월별 화면이 당시 예산과 실지출을 고정 기록으로 보여준다 → 초과 행의 주 액션이 `어디서 썼는지 보기`(딥링크) → `다음 달 계획 만들기`로 현재 예산 복사 → 지난달 실지출·3개월 평균 참고 → 새 달 시작 시 해당 버전 활성화, 과거 버전 보존.
...
- **공수**: L
- **의존**: Memory·Graph 목록 API에 페이지 상한이 없다(`apps/api/src/memory/memory.controller.ts:59-77`, `apps/api/src/graph/graph.controller.ts:53-91`). 사용자 표면을 열기 전에 페이지네이션이 먼저다.
...
- **재사용하는 기존 자산**: 3개 소스 전부 이미 호출 중이다 — `useDeclineList()`(`apps/web/src/components/widgets/decline-banner.tsx:32`), `useTransactions({status:"pending_review"})`(`apps/web/src/app/(app)/dashboard/page.tsx:206`), `GET /v1/card-sms-events?status=parse_failed,quarantined`(`:216`). **새 백엔드 없이 구현 가능하다.**
...
| C-8 할 일 탭 신설 + `/more` 3그룹 재편 + 아바타 메뉴 확장 | M + S | 하단 탭 구조 변경은 사용자 재학습 비용이 든다. 정기 지출(S4)과 예산 원장이 홈·`/more`의 정보량을 바꾸므로, **표면이 확정된 뒤 한 번에** 옮겨야 재학습이 한 번으로 끝난다 | S4 종료 후 IA 개편을 한 덩어리로 |
...
| Memory·Graph 목록 API 페이지 상한 없음 | `apps/api/src/memory/memory.service.ts:181` · `apps/api/src/graph/graph.service.ts:255` | 사용자 표면이 없어 현재 노출이 제한적이다. **단 C-6의 선행 조건이므로 그 직전에 반드시 처리** |
